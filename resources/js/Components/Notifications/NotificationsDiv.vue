...
<template>
    ...
    <div class="flex flex-col items-center justify-center w-full space-y-2">
        ...
        <div v-if="authStore.user"
            class="flex bg-white rounded-[20px] p-4 w-full items-center align-middle justify-between"
            v-for="notification in authStore.user.notifications" :key="notification.id">

            <div class="flex justify-start items-center">
                <img src="/assets/img/adduser.png" alt="Marcos Boiga" class="p-2 " />
                <div class="flex flex-col justify-start items-start w-full">
                    ...
                    <p class=" text-gray-400 text-xs">
                      ...
                      <!-- get the date and the hour by "2024-04-26T16:58:13.000000Z" and the format DD/MM/YY -->
                        {{ notification.created_at.split('T')[0].split('-')[2] }}/{{ notification.created_at.split('T')[0].split('-')[1] }}/{{ notification.created_at.split('T')[0].split('-')[0] }}
                         a les {{ notification.created_at.split('T')[1].split(':')[0] }}:{{ notification.created_at.split('T')[1].split(':')[1] }}
                    </p>
                </div>
            </div>
